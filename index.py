...
go_work_all_img = cv2.imread('targets/all-btn.png')
...
def clickBtn(img,name=None, timeout=3, trashhold = ct['default'], btn_is_list=False):
...
def clickButtons():
    buttons = positions(go_work_all_img, trashhold=ct['go_to_work_btn'])
...
def goToHeroes():
    if clickBtn(arrow_img, btn_is_list=True):
        global login_attempts
        login_attempts = 0

    # time.sleep(5)
    clickBtn(hero_img, btn_is_list=True)
    # time.sleep(5)

def goToGame():
    # in case of server overload popup
    clickBtn(x_button_img, btn_is_list=True)
    # time.sleep(3)
    clickBtn(x_button_img, btn_is_list=True)

    clickBtn(teasureHunt_icon_img, btn_is_list=True)

def refreshHeroesPositions():
    clickBtn(arrow_img, btn_is_list=True)
    clickBtn(teasureHunt_icon_img, btn_is_list=True)
    # time.sleep(3)
    clickBtn(teasureHunt_icon_img, btn_is_list=True)
...
def main():
    ...
    while True:
        ...
        if now - last["login"] > t['check_for_login'] * 60:
            ...
            sys.stdout.flush()
            last["login"] = now
            for _ in positions(connect_wallet_btn_img):
                login()
